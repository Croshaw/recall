...
public class LineNumbersForRTB : Control
{
    // Constants
    ...
    [AccessedThroughProperty("RTParent")] private RichTextBox pParent = null;
    [AccessedThroughProperty("Timer")] private Timer pTimer = null;
    private bool pAutoSizing = true;
    private Size pAutoSizingSize = new(0, 0);
    ...
    private Rectangle pContentRectangle = new();
    ...
    private Size pLineNumbersOffset = new(0, 0);
    ...
    private readonly List<LineNumberItem> pLineNumberItems = new();
    ...
    private Point pPointInMe = new(0, 0);
    private Point pPointInParent = new(0, 0);
    ...
    private void FindStartIndex(ref int min, ref int max, ref int target)
    {
        ...
        {
            var pos = RTParent.GetPositionFromCharIndex((max + min) / 2).Y;
            if (pos == target)
            {
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        e.Graphics.TextRenderingHint =
            pLineNumbersAntiAlias ? TextRenderingHint.AntiAlias : TextRenderingHint.SystemDefault;
        ...
        var gpBorderLines = new GraphicsPath(FillMode.Winding);
        var gpMarginLines = new GraphicsPath(FillMode.Winding);
        var gpLineNumbers = new GraphicsPath(FillMode.Winding);
        if (DesignMode)
        {
            var reminderToShow = string.Empty;
            if (RTParent == null)
                reminderToShow = "-!- Set ParentRichTextBox -!-";
            else if (pLineNumberItems.Count == 0) reminderToShow = "LineNumbers (  " + RTParent.Name + "  )";
            if (reminderToShow.Length > 0)
            {
                ...
                var textSize = e.Graphics.MeasureString(reminderToShow, Font, new Point(0, 0), format);
                ...
                var reminderRectangle = new Rectangle((int)Math.Round(Width / 2.0 - textSize.Height / 2.0),
                ...
                if (pAutoSizing)
                {
                    reminderRectangle.Inflate((int)Math.Round(textSize.Height * 0.2),
                        (int)Math.Round(textSize.Width * 0.1));
        ...
        if (pLineNumberItems.Count > 0)
            ...
            using (var pen = new Pen(pGridLinesColor, pGridLinesThickness) { DashStyle = pGridLinesStyle })
            {
                var format = new StringFormat
                {
                ...
                for (var i = 0; i <= pLineNumberItems.Count - 1; i++)
                {
                    // Background gradient
                    if (pGradientShow)
                        using (var brush = new LinearGradientBrush(pLineNumberItems[i].Rectangle,
                    ...
                    if (pGridLinesShow)
                    {
                        ...
                        var rect = new Rectangle((int)Math.Round(-pGridLinesThickness), pLineNumberItems[i].Rectangle.Y,
                    ...
                    if (pLineNumbersShow)
                    {
                        ...
                        var textSize = e.Graphics.MeasureString(textToShow, Font, point, format);
                        // Text alignment
                        point = GetAlignmentPoint(textSize, pLineNumberItems[i]);
                        // Text clipping
                        var itemClipRectangle = new Rectangle(point, textSize.ToSize());
        ...
        var left = new Point((int)Math.Round(Math.Floor(pBorderLinesThickness / 2.0)),
            (int)Math.Round(Math.Floor(pBorderLinesThickness / 2.0)));
        var right = new Point((int)Math.Round(Width - Math.Ceiling(pBorderLinesThickness / 2.0)),
            (int)Math.Round(Height - Math.Ceiling(pBorderLinesThickness / 2.0)));
        // Border lines
        var borderLinesPoints = new Point[5];
        ...
        if (pBorderLinesShow)
            using (var pen = new Pen(pBorderLinesColor, pBorderLinesThickness) { DashStyle = pBorderLinesStyle })
            {
        ...
        if (pMarginLinesShow && pMarginLinesSide > LineNumberDockSide.None)
        {
            ...
            using (var pen = new Pen(pMarginLinesColor, pMarginLinesThickness) { DashStyle = pMarginLinesStyle })
            {
                ...
                if ((pMarginLinesSide == LineNumberDockSide.Right) | (pMarginLinesSide == LineNumberDockSide.Height))
                {
                    e.Graphics.DrawLine(pen,
                        new Point((int)Math.Round(Width - Math.Ceiling(pMarginLinesThickness / 2.0)), 0),
        ...
        if (region.GetBounds(e.Graphics).IsEmpty)
        {
            ...
            gpBorderLines.AddLines(borderLinesPoints);
            gpBorderLines.CloseFigure();
            using (var pen = new Pen(pBorderLinesColor, 1f) { DashStyle = DashStyle.Solid })
            {
                gpBorderLines.Widen(pen);
    ...
    private void UpdateSizeAndPosition()
    {
        if (!AutoSize && !((Dock == DockStyle.Bottom) | (Dock == DockStyle.Fill) | (Dock == DockStyle.Top)))
        {
            ...
            if (pAutoSizing)
            {
                if (RTParent == null)
                {
                    if (pAutoSizingSize.Width > 0) zNewSize.Width = pAutoSizingSize.Width;
                    if (pAutoSizingSize.Height > 0) zNewSize.Height = pAutoSizingSize.Height;
                    Size = zNewSize;
                }
                ...
                else if (pDockSide != LineNumberDockSide.None)
                {
                    ...
                    if (pDockSide == LineNumberDockSide.Left) zNewLocation.X = RTParent.Left - zNewSize.Width - 1;
                    if (pDockSide == LineNumberDockSide.Right) zNewLocation.X = RTParent.Right + 1;
    ...
    private void UpdateVisibleLineNumberItems()
    {
        ...
        if (RTParent != null && RTParent.Text != "")
        {
            ...
            else
            {
                ...
                var startIndex = 0;
                var a = RTParent.Text.Length - 1;
                var y = pPointInParent.Y;
                ...
                startIndex = Math.Max(0,
                    Math.Min(RTParent.Text.Length - 1, RTParent.Text.Substring(0, startIndex).LastIndexOf('\n') + 1));
                ...
                {
                    var point = RTParent.GetPositionFromCharIndex(startIndex);
                    startIndex += Math.Max(1, split[a].Length + 1);
                    if (point.Y + pParentInMe > Height) break;
                    ...
                    pLineNumberItems.Add(new LineNumberItem(a + 1,
                        new Rectangle(0, point.Y - 1 + pParentInMe, Width, 1)));
                    if (pParentIsScrolling && DateTime.Now.Ticks > timeSpan.Ticks + ScrollTimeoutTicks)
                    {
                        ...
                        if (pLineNumberItems.Count == 1) pLineNumberItems[0].Rectangle.Y = 0;
                ...
                if (pLineNumberItems.Count == 0) return;
                ...
                if (a < split.Length)
                {
                    ...
                    var point = RTParent.GetPositionFromCharIndex(Math.Min(startIndex, RTParent.Text.Length - 1));
                ...
                for (a = 0; a <= pLineNumberItems.Count - 2; a++)
                    pLineNumberItems[a].Rectangle.Height = Math.Max(1,
                        pLineNumberItems[a + 1].Rectangle.Y - pLineNumberItems[a].Rectangle.Y);
                ...
                pLineNumbersFormat =
                    string.Empty.PadRight(
                        pLineNumbersShowAsHexadecimal
                            ? split.Length.ToString("X").Length
                            : split.Length.ToString().Length, '0');
            }
    ...
    [Description("Use this property to enable the control to act as an overlay ontop of the RichTextBox.")]
    ...
    [Category("Additional Behavior")]
    [Description(
        "Use this to apply Anti-Aliasing to the LineNumbers (high quality). Some fonts will look better without it, though.")]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public bool LineNumbersAntiAlias
    {
        get => pLineNumbersAntiAlias;
        set
        {
    ...
    {
        get => pLineNumbersShowAsHexadecimal;
        set
        {
    ...
    [Category("Additional Behavior")]
    [Description(
        "Use this to set whether the LineNumbers are allowed to spill out of their item-area, or should be clipped by it.")]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public bool LineNumbersClippedByItemRectangle
    {
        get => pLineNumbersClipByItemRectangle;
        set
        {
    ...
    [Description(
        "Use this to set whether the LineNumbers should have leading zeroes (based on the total amount of textlines).")]
    [Category("Additional Behavior")]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    public bool LineNumbersLeadingZeroes
    {
        get => pLineNumbersShowLeadingZeroes;
        set
        {
    ...
    [Category("Additional Behavior")]
    [Description(
        "The BackgroundGradient is a gradual blend of two colors, shown in the back of each LineNumber's item-area.")]
    [DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden)]
    ...
    {
        [DebuggerNonUserCode] get => pParent;
    ...
    private Timer Timer
    {
        [DebuggerNonUserCode] get => pTimer;
        [MethodImpl(MethodImplOptions.Synchronized)]
        [DebuggerNonUserCode]
        set
        {
            EventHandler handler = TimerTick;
            if (pTimer != null) pTimer.Tick -= handler;
            pTimer = value;
            if (pTimer != null) pTimer.Tick += handler;
        }
    }
